...
def test_remove_products_from_cart(page, home_page, products_page, cart_page):
    ...
    # Click second product add-to-cart robustly
    try:
        btn = products_page.page.locator(".features_items .add-to-cart").nth(2)
        btn.scroll_into_view_if_needed(timeout=2000)
        btn.click(timeout=2000)
    except Exception:
        try:
            products_page.page.locator(".features_items .add-to-cart").nth(2).click(force=True, timeout=2000)
        except Exception:
            try:
                products_page.page.locator(".features_items .add-to-cart").nth(2).evaluate("el => el.click()")
            except Exception:
                pass
