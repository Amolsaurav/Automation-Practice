...
def load_env_file():
    ...
    try:
        ...
        print(f"[AI Fix] Could not read .env file: {exc}")
...
if os.path.exists(os.path.join(PROJECT_DIR, ".env")):
    ...
    if env_key:
        print(f"[AI Fix] Using GEMINI_API_KEY from .env file: {env_key[:6]}...{env_key[-2:]}")
...
if not api_key:
    print("[AI Fix] GEMINI_API_KEY not set in environment - skipping automatic test fixes.")
    sys.exit(0)

try:
    gemini = genai.Client(api_key=api_key)
except Exception as exc:
    print(f"[AI Fix] Failed to initialize Gemini client: {exc}")
    traceback.print_exc()
    sys.exit(0)

if not os.path.exists(RESULTS_FILE):
    print(f"[AI Fix] {RESULTS_FILE} not found - nothing to fix.")
    sys.exit(0)
...
Return only the corrected Python code, nothing else."""
    ...
    try:
        ...
        print(f"[AI Fix] Gemini response rejected for {issue['test']}: {exc}")
        traceback.print_exc()
        continue
